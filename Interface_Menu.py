...
def Tela_Nivel():
	...
	while run:
		...
		if nivel1_button.draw(screen): # detectar a seleção do nivel 1
			tamanho_Matriz = (27,17)   # define tamanho
			interaction+=1
			
		if nivel2_button.draw(screen): # detectar a seleção do nivel 2
			tamanho_Matriz = (37,27)   # define tamanho
			interaction+=1		
		if nivel3_button.draw(screen): # detectar a seleção do nivel 3
		...
		if interaction > 0: #finaliza tela
			...
			pygame.quit()
			pygame.time.delay(100)
			Tela_criar_labirinto() # função para definir o modo de criação do labirinto
...
def tela_Resolver(labirinto):
	...
	while run: #looping onde o jogo comeca
		...
		if manual_button.draw(screen): # detectar a seleção de resolução manual
			pygame.quit()
			pygame.time.delay(100)
		...
		if auto_button.draw(screen): # detectar a seleção de resolução automatico
			pygame.quit()
			pygame.time.delay(100)
			jogo = interface(labirinto)
			resposta, movimentos = jogo.to_execute_Automatico()
			
			if resposta: # se o labirinto não tiver solucao (caminho ate o ponto final)
				pygame.quit()
				pygame.time.delay(100)
				tela_Sem_Solucao()
				return
...
def tela_Ganhou(movimento):
	...
	pygame.font.init() # instruções de texto
	font_instrucoes2 = pygame.font.SysFont('Rockwell Extra Bold', 90) # definição de fonte
	...
	while run: #looping onde o jogo comeca
		...
		if continuar_button.draw(screen):  # detectar a seleção do continuar
			pygame.quit()
			pygame.time.delay(100)
...
def tela_jogar_novamente():
	...
	while run:
		...
		if yes_button.draw(screen): # detectar a seleção do sim
			pygame.quit()
			pygame.time.delay(100)
			Tela_inicial()
			return
		if no_button.draw(screen): # detectar a seleção do nao
			quit()
...
# tela_Desistiu = Se o jogador desistir, ele terá o opção de sair ou resolver o labirinto de forma automatica
def tela_Desistiu(labirinto):
	...
	while run:
		...
		if resolver_button.draw(screen):  # detectar a seleção para resolver automaticamente
			pygame.quit()
			pygame.time.delay(100)
			jogo = interface(labirinto)
			resposta, movimentos = jogo.to_execute_Automatico()
